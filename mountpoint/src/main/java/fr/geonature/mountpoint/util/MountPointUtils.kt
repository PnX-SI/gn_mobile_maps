...
object MountPointUtils {
    ...
    fun getInternalStorage(context: Context): MountPoint {
        ...
        if (externalStorage.isNullOrBlank()) {
            val mountPoint = buildMountPoint(
                context.getExternalFilesDir(null)?.absolutePath?.split("/Android/data")
                    ?.firstOrNull() ?: Environment.getExternalStorageDirectory().absolutePath,
                MountPoint.StorageType.INTERNAL
            )

            Logger.debug { "internal storage from API: $mountPoint" }
        ...
        Logger.debug { "internal storage from system environment: $mountPoint" }
    ...
    fun getExternalStorage(
        ...
        if (externalMountPoint == null) {
            Logger.debug { "external storage not found" }
        } else {
            Logger.debug { "external storage found: $externalMountPoint" }
        }
     ...
     *
     * @see mountPointsFromSystemEnv
     * @see mountPointsFromVold
     * @see mountPointsFromProcMounts
     */
    private fun getMountPoints(context: Context): List<MountPoint> {
        ...
        get() {
            ...
            if (!secondaryStorage.isNullOrBlank()) {
                ...
                for (path in paths) {
                    ...
                    if (mountPoint != null) {
                        Logger.debug { "mount point found from system environment: $mountPoint" }
            ...
            Logger.debug { "mount points found from system environment: " + mountPoints.size }
    ...
    private val mountPointsFromVold: List<MountPoint>
        get() {
            ...
            try {
                ...
                Logger.warn { fnfe.message ?: "'vold.fstab' not found" }
            }

            Logger.debug { "mount points found from 'vold.fstab': " + mountPoints.size }
    ...
    private val mountPointsFromProcMounts: List<MountPoint>
        get() {
            ...
            try {
                ...
                Logger.warn { fnfe.message ?: "'/proc/mounts' not found" }
            }

            Logger.debug { "mount points found from '/proc/mounts': " + mountPoints.size }
    ...
    private fun buildMountPoint(
        ...
        return MountPoint(runCatching {
            File(mountPath.ifBlank { "/" }).canonicalFile
        }.getOrDefault(File(mountPath.ifBlank { "/" })),
            storageType
        )
    }
}
